...
// ---------------------------------------------------------------------------
test("§5.2.1 — deux guichetiers sur le même siège : un seul billet émis", () => {
  const fixture = seedFixture();
  const [seat] = seatsOfChannel(fixture.tripId, "GUICHET", 1);

  const first = sellAtCounter(fixture, seat, { userId: fixture.guichetierId });
...
test("§5.2.1 bis — verrouillage concurrent : un seul verrou obtenu", () => {
  const fixture = seedFixture();
...
// ---------------------------------------------------------------------------
test("§5.2.2 — vente hors-ligne rejouée deux fois : un seul billet", () => {
  const fixture = seedFixture();
  const [seat] = seatsOfChannel(fixture.tripId, "GUICHET", 1);
  const clientOpId = "pos-device-1:op-42";

  const first = sellAtCounter(fixture, seat, { clientOpId });
  // Le POS n'a pas reçu l'accusé et rejoue l'opération au retour du réseau.
  const replay = posSell({
    tripId: fixture.tripId,
    seatNumbers: [seat],
    passengers: [{ seatNumber: seat, name: "Passager Guichet" }],
...
test("§5.2.2 bis — le quota guichet borne la vente hors-ligne", () => {
  ...
  const fixture = seedFixture({ quotas: { GUICHET: 2, EN_LIGNE: 56, RESERVE_COMPAGNIE: 2 } });
  const seats = seatsOfChannel(fixture.tripId, "GUICHET", 5);
  assert.equal(seats.length, 2, "le quota guichet ne contient que 2 sièges");

  const sessionId = openCashSession({
    agencyId: fixture.agencyId,
  ...
  sellAtCounter(fixture, seats[0], { sessionId });
  sellAtCounter(fixture, seats[1], { sessionId });

  const enLigne = seatsOfChannel(fixture.tripId, "EN_LIGNE", 1)[0];
  assert.throws(
...
test("§5.2.3 — deux acheteurs sur un siège remis en vente : un seul aboutit", async () => {
  ...
  const listing = listForResale({ ticketId: ticket.id, actorPhone: ticket.passenger_phone });
  ...
  for (const phone of acheteurs) {
    ...
    holdSeats({ tripId: fixture.tripId, seatNumbers: [support], holdId });
    const { booking } = createBooking({
      tripId: fixture.tripId,
      holdId,
      buyerPhone: phone,
...
test("§5.2.4 — double clic sur le paiement : un seul débit", async () => {
  ...
  const count = getDb()
    .prepare(`SELECT COUNT(*) AS n FROM payments WHERE booking_id = ?`)
...
test("§5.2.5 — webhook jamais reçu : INDETERMINE, siège verrouillé, ticket support", async () => {
  ...
  const seatRow = getDb()
    .prepare(`SELECT status FROM trip_seats WHERE trip_id = ? AND seat_number = ?`)
...
test("§5.2.6 — ancien QR après revente : refus avec le nouveau titulaire", async () => {
  ...
  const listing = listForResale({ ticketId: ancien.id, actorPhone: ancien.passenger_phone });
  const support = seatsOfChannel(fixture.tripId, "EN_LIGNE", 1)[0];
  holdSeats({ tripId: fixture.tripId, seatNumbers: [support], holdId: "h-rachat" });
  const { booking } = createBooking({
    tripId: fixture.tripId,
    holdId: "h-rachat",
  ...
  const outcome = scanTicket({ tripId: fixture.tripId, rawQr: ancienQr });
  ...
  const manifest = buildManifest(fixture.tripId);
...
// ---------------------------------------------------------------------------
test("§5.2.7 — même QR scanné deux fois : avertissement daté", () => {
  const fixture = seedFixture();
  const [seat] = seatsOfChannel(fixture.tripId, "GUICHET", 1);
  const vente = sellAtCounter(fixture, seat);
  const qr = vente.tickets[0].qr_signature;

  const premier = scanTicket({ tripId: fixture.tripId, rawQr: qr, deviceId: "terminal-1" });
  assert.equal(premier.result, "ACCEPTE");

  const second = scanTicket({ tripId: fixture.tripId, rawQr: qr, deviceId: "terminal-1" });
...
test("§2.7 — le QR se vérifie hors-ligne, sans accès à la base", () => {
  const fixture = seedFixture();
  const [seat] = seatsOfChannel(fixture.tripId, "GUICHET", 1);
  const ticket = sellAtCounter(fixture, seat).tickets[0];
  const company = getCompany(fixture.companyId);
...
// ---------------------------------------------------------------------------
test("§5.2.8 — fermeture de caisse : écart exact, clôture non rejouable", () => {
  const fixture = seedFixture();
  ...
  for (const seat of seats) sellAtCounter(fixture, seat, { sessionId: session.id });

  const avant = cashSessionSummary(session.id);
  ...
  const fermeture = closeCashSession({
  ...
  assert.throws(
    () =>
      posSell({
        tripId: fixture.tripId,
        seatNumbers: seatsOfChannel(fixture.tripId, "GUICHET", 1),
        passengers: [{ seatNumber: seatsOfChannel(fixture.tripId, "GUICHET", 1)[0], name: "Tardif" }],
  ...
  const alerte = getDb()
    .prepare(`SELECT COUNT(*) AS n FROM alerts WHERE kind = 'ECART_CAISSE'`)
    .get() as { n: number };
