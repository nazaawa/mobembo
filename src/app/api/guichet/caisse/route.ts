...
export const GET = authed(["GUICHETIER", "GERANT_AGENCE"], async ({ session }) => {
  ...
  const open = openSessionFor(session.userId, session.agencyId);
  return { session: open ? cashSessionSummary(open.id) : null };
});
...
export const POST = authed(
  ...
  async ({ request, session, device }) => {
    ...
    return { session: cashSessionSummary(opened.id) };
  },
);
