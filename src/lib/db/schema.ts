/**
 * Schéma de la base — source de vérité.
 *
 * Le SQL vit dans un module TypeScript plutôt que dans un fichier lu au
 * démarrage : un `readFileSync` au runtime force Next à embarquer tout le
 * projet dans le bundle serveur. `npm run schema:export` en produit la version
 * `.sql` livrable (§4.2 « export du schéma »).
 *
 * Dialecte MySQL 8 (InnoDB). Les identifiants et colonnes indexées ou UNIQUE
 * — TEXT sans borne sous SQLite — deviennent VARCHAR bornés : InnoDB exige
 * une longueur de clé connue. REAL devient DOUBLE. L'index partiel SQLite
 * `UNIQUE(agency_id, sequence_number) WHERE sequence_number IS NOT NULL`
 * (non supporté par MySQL) est recréé par une colonne générée qui vaut NULL
 * tant que sequence_number est NULL — MySQL autorise plusieurs NULL dans un
 * index UNIQUE, ce qui reproduit exactement la même sémantique.
 *
 * `CREATE INDEX` n'a pas de variante `IF NOT EXISTS` en MySQL : les
 * instructions d'index en fin de fichier sont exécutées une par une par
 * `src/lib/db/index.ts`, qui ignore l'erreur "Duplicate key name" (1061) au
 * redémarrage.
 */
